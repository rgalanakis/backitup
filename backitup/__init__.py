"""backitup is a module for easily backing up files. It is designed for
ease of use for python programmers, by doing a few simple things
clearly.

General Use
&&&&&&&&&&&

- Create the python file that will call backitup. Generally this will
  hard-code some configuration information, such as the backup
  directory, source file paths, etc. It can also do stuff like
  configure logging.
...
A demonstration of a common use case (how I use it on my personal machine
and server) is provided at the end of this file.

Notes
&&&&&

All 'configuration' is handled by passing arguments into the functions.
The module-level constants are for defining default values
only.
